...
def _build_research_prompt(digest: dict[str, Any], existing_report: str | None = None) -> str:
    ...
    return (
        ...
        "- Usa Markdown estricto. Si la respuesta no está en Markdown correctamente formateado, se considera inválida.\n"
        "- Usa encabezados (## para secciones principales, ### para subsecciones).\n"
        "- Usa listas con guiones (-) para los puntos dentro de cada sección.\n"
        "- Usa negrita (**texto**) para las etiquetas clave (ej. **Acción:**, **Por qué:**, **Referencia:**).\n"
        "- Separa cada sección principal con una línea horizontal (---).\n"
        "- Evita párrafos largos: desglosa la información en listas y subsecciones cortas.\n"
        "- NO uses texto plano sin estructura.\n"
        "- NO uses HTML.\n"
        "- NO uses notación LaTeX ni llaves {} en el informe.\n"
        "- Expresa fórmulas en texto plano (ejemplo: 'v_media = d / t = 5.42 m/s').\n"
        "- No incluyas ningún símbolo que pueda interpretarse como una variable de plantilla.\n\n"
        "ESTRUCTURA DE EJEMPLO para la sección de Recomendaciones:\n"
        "## Recomendaciones para esta semana\n\n"
        "### 1. Título de la recomendación\n"
        "- **Acción:** descripción concisa\n"
        "- **Por qué:** justificación basada en datos\n"
        "*(Sección X.Y)*\n\n"
        "---\n\n"
        "## ⚠️ Notas importantes\n\n"
        "- Nota breve en lista\n\n"
